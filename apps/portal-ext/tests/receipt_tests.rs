...
async fn writes_receipt_and_updates_root() {
    ...
    let receipt = write_receipt(&state, "test", "guardian-alpha", &payload).expect("receipt write");
...
async fn receipt_includes_merkle_path_on_subsequent_entries() {
    ...
    let _ = write_receipt(&state, "test", "guardian-alpha", &payload).expect("receipt write");
    let second = write_receipt(&state, "test", "guardian-alpha", &payload).expect("receipt write");
